...
function determine_gpu_use_host() {
    ...
    if [ ${USE_GPU_HOST} -eq 1 ]; then
        ...
        if [[ -x "$(which nvidia-container-toolkit)" ]]; then
            ...
            DOCKER_RUN="sudo nvidia-docker run"
        else
            USE_GPU_HOST=0
...
function remove_existing_dev_container() {
    if sudo docker ps -a --format '{{.Names}}' | grep -q "${APOLLO_DEV}"; then
        sudo docker stop "${APOLLO_DEV}" >/dev/null
        sudo docker rm -v -f "${APOLLO_DEV}" >/dev/null
    fi
}
...
function docker_restart_volume() {
    ...
    docker_pull "${image}"
    sudo docker run -itd --rm --name "${container}" "${image}"
}
...
function post_run_setup() {
    if [ "${USER}" != "root" ]; then
        sudo docker exec -u root "${APOLLO_DEV}" bash -c '/apollo/scripts/docker_start_user.sh'
    fi
}
